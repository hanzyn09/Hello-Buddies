...
import tamagotchi.service.TamagotchiServiceImpl;
@Slf4j
@Component
public class FileUtils {
    ...
    public List<TamagotchiFileDto> parseFileInfo(int tamagotchiId, MultipartHttpServletRequest request) throws Exception {
        if (ObjectUtils.isEmpty(request)) {
            return null;
        }
        
        List<TamagotchiFileDto> fileInfoList = new ArrayList<>();
        ...
        while(fileTagNames.hasNext()) {
            ...
            for (MultipartFile file : files) {
                ...
                if (!file.isEmpty()) {
                    ...
                    // 파일 정보를 리스트에 저장 
                    TamagotchiFileDto dto = new TamagotchiFileDto();
                    dto.setTamagotchiId(tamagotchiId);
                    dto.setImageUrl(file.getOriginalFilename());
